...
// from the fallback branch. The actual implementation accepts an optional options
// object; we cast it here so strict mode is satisfied without touching the JS file.
type NotifyFn = (message: string, options?: { variant?: string; duration?: number }) => void;

// ─── Constants ───────────────────────────────────────────

const PARTNER_TYPES = ['Individual', 'Firm', 'Company'] as const;
const PARTNER_STATUSES = ['Active', 'Inactive', 'Suspended'] as const;
const COMMISSION_TYPES = ['Percentage', 'Fixed', 'Slab'] as const;

type CommissionType = (typeof COMMISSION_TYPES)[number];

interface TabDef {
    id: 'partners' | 'commissions' | 'payouts';
    label: string;
    icon: typeof Users2;
}
...
export default function ChannelPartnersPage() {
    ...
    // Cast to the full signature; the runtime implementation supports options.
    const notify = _notify as NotifyFn;
    const [loading, setLoading] = useState(true);
    const [activeTab, setActiveTab] = useState<TabDef['id']>('partners');
    ...
    const [batchToComplete, setBatchToComplete] = useState<PayoutBatchRow | null>(null);
    ...
    const handleOnboard = async (e: FormEvent<HTMLFormElement>) => {
        ...
        const commissionType = (f.elements.namedItem('commissionType') as HTMLSelectElement).value;
        const payload = {
    ...
    const handleCreateCommission = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        const f = e.currentTarget;
        const partnerId = Number((f.elements.namedItem('partnerId') as HTMLSelectElement).value);
        const bookingIdVal = (f.elements.namedItem('bookingId') as HTMLInputElement).value;
        const dealIdVal = (f.elements.namedItem('dealId') as HTMLInputElement).value;
        const payload: { partnerId: number; bookingId?: number; dealId?: number } = { partnerId };
        if (bookingIdVal) payload.bookingId = Number(bookingIdVal);
        if (dealIdVal) payload.dealId = Number(dealIdVal);
        setSubmitting(true);
    ...
    const toggleCommissionSelect = (id: number) => {
        setSelectedCommissionIds((prev) =>
            prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]
        );
    };

    const handleCreateBatch = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        const f = e.currentTarget;
        ...
        const res = await createPayoutBatch({
            batchName: (f.elements.namedItem('batchName') as HTMLInputElement).value,
    ...
    const handleCompleteBatch = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        ...
        const res = await completePayoutBatch({
            ...
            utr: (f.elements.namedItem('utr') as HTMLInputElement).value,
        });
        setSubmitting(false);
        if (res.success) {
            notify(
                `Batch completed: ${res.data.commissionsPaid} commission(s) marked Paid`,
                { variant: 'success' },
            );
    ...
    return (
        <div className="space-y-6 animate-[fade-in_0.5s_ease-out] min-w-0">
            ...
            {activeTab === 'partners' && (
                <PartnersTab partners={filteredPartners} search={search} setSearch={setSearch} />
            )}
            ...
            <Modal isOpen={showCommission} onClose={() => setShowCommission(false)} title="Compute Commission">
                <form className="space-y-4" onSubmit={handleCreateCommission}>
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit" disabled={submitting} className="px-6 py-2.5 bg-accent hover:bg-accent-hover text-white rounded-xl text-sm font-semibold transition-all disabled:opacity-50">
                            {submitting ? 'Computing...' : 'Create'}
                        </button>
                    </div>
                </form>
            </Modal>
                ...
                <form className="space-y-4" onSubmit={handleCreateBatch}>
                    ...
                    <div className="flex items-center justify-between pt-2">
                        ...
                        <div className="flex gap-3">
                            ...
                            <button type="submit" disabled={submitting || selectedCommissionIds.length === 0} className="px-6 py-2.5 bg-accent hover:bg-accent-hover text-white rounded-xl text-sm font-semibold transition-all disabled:opacity-50">
                ...
                {batchToComplete && (
                    <form className="space-y-4" onSubmit={handleCompleteBatch}>
                        <p className="text-sm text-muted">
                            ...
                            {batchToComplete.commissionCount} included commission(s) as{' '}
                            <strong className="text-foreground">Paid</strong>. This cannot be undone.
                        </p>
                        <div>
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            <button type="button" onClick={() => setBatchToComplete(null)} className="px-4 py-2.5 rounded-xl text-sm text-muted hover:text-foreground hover:bg-surface-hover transition-colors">
                                Cancel
                            </button>
                            <button type="submit" disabled={submitting} className="px-6 py-2.5 bg-success hover:bg-success/90 text-white rounded-xl text-sm font-semibold transition-all disabled:opacity-50">
                                {submitting ? 'Completing...' : 'Mark Paid'}
                            </button>
                        </div>
                    </form>
                )}
...
function MetricsRow({ metrics }: { metrics: PartnerMetrics | null }) {
...
function PartnersTab({ partners, search, setSearch }: PartnersTabProps) {
    return (
                            ...
                            <tbody>
                                {partners.map((p) => (
                                    <tr key={p.id}>
                                        ...
                                        <td>
                                            <span className={`badge ${partnerStatusColor[p.status] ?? ''}`}>
                                                {p.status}
                                            </span>
                                        </td>
...
function CommissionsTab({ commissions, onApprove, onCreate }: CommissionsTabProps) {
    return (
        <div className="space-y-4">
            <div className="flex justify-end">
                <button
                    onClick={onCreate}
                    className="flex items-center gap-2 px-4 py-2 bg-surface border border-border hover:border-accent/40 text-foreground rounded-xl text-sm font-medium transition-all"
                >
                            ...
                            <tbody>
                                {commissions.map((c) => (
                                    <tr key={c.id}>
                                        ...
                                        <td>
                                            <span className={`badge ${commissionStatusColor[c.status] ?? ''}`}>
                                                {c.status}
                                            </span>
                                        </td>
                                        <td>
                                            {c.status === 'Pending' && (
                                                <button
                                                    ...
                                                    className="text-xs font-medium text-info hover:text-info/80 flex items-center gap-1"
                                                >
...
interface PayoutsTabProps {
    ...
    batchable: CommissionRow[];
    onCreate: () => void;
    onComplete: (batch: PayoutBatchRow) => void;
}

function PayoutsTab({ batches, batchable, onCreate, onComplete }: PayoutsTabProps) {
    return (
        <div className="space-y-4">
                        ...
                        <div key={b.id} className="glass-card p-4">
                            ...
                            {b.status !== 'Completed' && (
                                <button
                                    ...
                                    className="w-full flex items-center justify-center gap-1.5 px-3 py-2 bg-success/10 text-success border border-success/20 hover:bg-success/20 rounded-lg text-xs font-medium transition-colors"
                                >
...
function OnboardForm({ submitting, onSubmit, onCancel }: OnboardFormProps) {
