...
/**
 * Non-consuming look at a key's current state.
 *
 * Unlike {@link checkRateLimit}, this NEVER increments the counter and NEVER
 * creates a bucket — it just reports whether the key is presently over its
 * limit and when the window refills. Use it to *gate* an action before doing
 * the work that would consume a token, e.g. refusing a login attempt while an
 * account is in its failure-lockout window without itself counting as an
 * attempt.
 */
export function peekRateLimit(
  ...
  const now = Date.now();
  const entry = buckets.get(key);
  if (!entry || entry.resetAt <= now) return { blocked: false, reset: 0 };
  return { blocked: entry.count >= limit, reset: entry.resetAt };
}
...
export const RATE_LIMITS = {
  ...
  /** Reaction add/swap/remove. More permissive than normal sends. */
  react: { limit: 120, windowMs: 60_000 },
  /** Channel-partner portal login. 5 failed attempts per account within a
   *  15-minute window trips a 15-minute block (Req 7.2). Only *failed*
   *  attempts consume this budget; gate attempts with `peekRateLimit`. */
  cpLogin: { limit: 5, windowMs: 15 * 60_000 },
} as const;
